...
from modeling_finetune import Block, _cfg, PatchEmbed, RelativePositionBias, CrossSelfBlock, CrossSelfDecoderBlock
...
__all__ = [
    'conmim_small_patch_3',
]


class VisionTransformerForMaskedImageModeling(nn.Module):   
    def __init__(self, patch_size=16, in_chans=3, vocab_size=8192, embed_dim=768, encoder_depth=12, dim=200, mlp_dim=800,  
                 num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=None, init_values=None, attn_head_dim=None,
                 use_abs_pos_emb=True, use_rel_pos_bias=False, use_shared_rel_pos_bias=False, init_std=0.02, use_lm_head=False, use_mlp_projectors = False):
        super().__init__()
...
@register_model
def conmim_small_patch_3(pretrained=False, **kwargs):      
    model = VisionTransformerForMaskedImageModeling( 
        patch_size=kwargs['patch_size'], in_chans=kwargs['patch_size']**2, embed_dim=kwargs['n_bands'], mlp_dim=kwargs['mlp_dim']*4, dim=kwargs['mlp_dim'], encoder_depth=kwargs['depth'], num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), vocab_size=8192, use_mlp_projectors=True, use_rel_pos_bias=kwargs['use_rel_pos_bias'], use_abs_pos_emb=kwargs['use_abs_pos_emb'], drop_path_rate=kwargs['drop_path_rate'])

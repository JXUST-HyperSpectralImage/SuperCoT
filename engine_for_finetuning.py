...
from utils import accuracy, grouper, count_sliding_window, sliding_window
import utils
import time
...
def test_pre(data_loader, model, device, predition=[], targets=[], outputs=[]):
    targets = []
    outputs = []
    predition=[]
    criterion = torch.nn.CrossEntropyLoss()
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        # compute output
        with torch.cuda.amp.autocast():
            output, out_sne = model(images)
            loss = criterion(output, target)
        
        outputs.extend(out_sne.tolist())
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, predition, targets, outputs

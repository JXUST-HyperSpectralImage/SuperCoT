...
class RandomResizedCropAndInterpolationWithTwoPic:
    ...
    def __call__(self, img):
        ...
        """                                                             #（i，j）为裁剪图片块左上角坐标，i为垂直 j为水平方向值，相当于x轴是垂直方向
        i, j, h, w = self.get_params(img, self.scale, self.ratio)       # h为图片的高度，w为图片宽度，（i+h，j+w）为图片右下角
        if isinstance(self.interpolation, (tuple, list)):
            interpolation = random.choice(self.interpolation)
        else:
            interpolation = self.interpolation
        if self.second_size is None:
            return F.resized_crop(img, i, j, h, w, self.size, interpolation)        #resized_crop       crop()：如果是tensor 则返回 img[..., top:top + height, left:left + width] （返回区域内所有tensor值）（h,w）
                                                                                    #                   如果是PIL Image  则返回 img.crop((left, top, left + width, top + height))  （输入左上角和右下角的坐标(x0, y0, x1, y1)，x代表水平方向） (w,h)                 
        else:                                                                       #                   resize():如果输入的size是一个整数则保持宽高比短边为输入size，如果输入的是一个元组如（22，32）就按照输入size进行resize

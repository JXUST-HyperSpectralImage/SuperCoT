...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    ...
                    lr_schedule_values=None, wd_schedule_values=None, alpha = None, gamma = None, maxsp_labels = None):
    model.train()
    ...
    for step, (batch) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        samples, samples_aug, bool_masked_pos, superpixel_label= batch
        bs = samples.shape[0]
        samples = samples.to(device, non_blocking=True)
        bool_masked_pos = bool_masked_pos.to(device, non_blocking=True)
        samples_aug = samples_aug.to(device, non_blocking=True)
        superpixel_label = superpixel_label.to(device, non_blocking=True)
        ...
        length = model.patch_embed.num_patches
        ...
        with torch.cuda.amp.autocast(): 
            ...
            superpixel_label = superpixel_label.reshape(bs, -1)    
            mask_superpixel_label = superpixel_label[bool_masked_pos]      
            superpixel_label = superpixel_label.reshape(-1) 
            unique_superpixel_labels = torch.unique(superpixel_label)
            if step == 0:
                cluster_centers = torch.zeros(maxsp_labels, bd).to(torch.half).to(device)
            for super_label in unique_superpixel_labels:                                        
                same_label_features = feat_full.reshape(-1, bd)[superpixel_label == super_label]
                cluster_center = same_label_features.mean(dim=0)
                cluster_centers[super_label] = gamma*cluster_centers[super_label] + (1-gamma)*cluster_center
            mask_superpixel_center = cluster_centers[mask_superpixel_label]  
            feat_full[bool_masked_pos] = mask_superpixel_center
